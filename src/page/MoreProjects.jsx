...
function MoreProjects() {
  ...
  return (
    <>
      <Navbar />
      <div id="Projects" ref={sectionRef} className="w-full relative pt-32">
        <div className="max-w-7xl mx-auto h-full flex flex-col items-center py-10 relative px-4">
          <button
            onClick={() => navigate("/")}
            className="self-start flex items-center gap-2 text-sm font-mulish text-accent hover:text-white transition-colors duration-300 mb-6"
          >
            <FaArrowLeft className="w-3 h-3" />
            Back to Home
          </button>

          <div ref={headingRef} className="w-full flex flex-col gap-4 items-center text-center">
            ...
            <p className="max-w-2xl text-gray-400 text-sm md:text-base mt-2 font-mulish leading-relaxed">
              A few more things I&apos;ve built and shipped.
            </p>
          </div>
                  ...
                  <div className="w-full md:w-1/2 h-80 flex flex-col items-start gap-3 px-8 ">
                    ...
                    <div className="w-full flex flex-wrap gap-2.5">
                      {project?.tags?.map((tag, tagIndex) => (
                        <span key={tagIndex} className="skill-chip !px-3 !py-1 text-xs">
                          #{tag}
                        </span>
                      ))}
                    </div>
